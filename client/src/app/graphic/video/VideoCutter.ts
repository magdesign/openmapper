...
export class VideoCutter {
    ...
    /**
     * creates a new Videomaterial. The difference to init is that you can set the startposition
     * @param targets 
     * @param video 
     * @param startPoint 
     */
    ...
    private static initCutterDragHandle(videoMaterial: IVideoMaterial, dragger: IDragHandler, target: IVideoMaterial): IDragHandler {
        ...
        const vertices = DimensionTransformer.fromFloatArrayToDimension(line.geometries[0].data.vertices);
        const spriteEdges = Edges.reorderLineEdgesForSprites(vertices);
    ...
    private static createCutterDragHandle(videoMaterial: IVideoMaterial, target: IVideoMaterial): IDragHandler {
        const cutter = DragHandler.createEdges(videoMaterial.positions, DragHandlerTypes.Cutter, (event) => {
            const activeDragHandler = videoMaterial.dragHandler
                        .filter((dh: IDragHandler) => dh.id === event.object.name)[0];
