...
export interface IDragHandler {
    ...
    line?: Line;
    sprites: Sprite[];
    fn: (event?: any) => void; // function which will be executed when draghandle event is thrown
    dragEventType: DragEventType; // event thrown by click or drag
...
export class DragHandler {
    ...
    public static createEdges(positions: IDimension[], type: DragHandlerTypes, fn: (event: any) => void): IDragHandler {
    ...
    public static updateByVecotor(dragHandle: IDragHandler, vector: IDimension): IDragHandler {
        dragHandle.sprites.map((sprite: Sprite): Sprite => {
            ...
            sprite.position.setZ(sprite.position.z + vector.z);
            return sprite;
        });
